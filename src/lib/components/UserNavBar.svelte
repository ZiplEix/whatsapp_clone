...
<script lang='ts'>
  ...
  import { pb } from "$lib/pocketbase";
    ...
    let editingProfil = false;
</script>

<nav transition:fly={{x:"-100%", duration: 200, opacity: 0.5}} class:hide={$page.url.pathname !== "/"}>
    <header>
    ...
    <section class="user-nav-container">
        {#if editingProfil}
            <form action="/?/updateUserInformation" method="post" class="profile-form" enctype="multipart/form-data" use:enhance={
                () => {
                    return () => {
                        editingProfil = false;
                    };
                ...
                <div class="input-container">
                    <span>Profil image</span>
                    <input type="file" name="photo" accept="image/png image.jpg image.gif image/svg image/webp">
                </div>

                <button type="submit" class="update-profiles">Update profile</button>
                <button on:click={() => (editingProfil = false)}>Cancel</button>
            </form>
        {:else}
            ...
            <div class="account-actions">
                <button on:click={() => editingProfil = true}>Edit profile</button>
