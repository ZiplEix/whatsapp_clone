...
<script lang='ts'>
    ...
    import { pb, conversationsStore } from "$lib/pocketbase";
...
{#if $pb}
    <nav class="hide" class:show={$page.url.pathname === "/"}>
        <header>
            ...
            <div class="user-information" on:click={() => dispatchEvent("openUserNav")}>
                <div class="user-image">
                    {#if authModel.photo && authModel.photo.length > 0}
                        <img src={$pb.files.getUrl(authModel, authModel.photo)} alt={authModel.name} />
                    {:else}
                        <FaUserCircle />
                    {/if}
                </div>
            ...
            <a href="/create-conversation" on:click={(event) => dispatchEvent("openCreateConversation", event)} class="create-conversation">
                <IoMdCreate />
            </a>
        </header>
        ...
        <section class="conversation-view">
            {#if $conversationsStore && $conversationsStore.length > 0}
                {@const sortedConversation = [...$conversationsStore].sort((a, b) => {
                    return (
                ...
                {#each sortedConversation as conversation}
                    {@const createdDate = conversation.messages.length > 0 ? new Date(conversation.messages[0].created) : new Date(conversation.created)}
                        ...
                        <div class="conversation-information">
                            <div class="conversation-image">
                                {#if conversation.photo && conversation.photo.length > 0}
                                    <img src={$pb.getFileUrl(conversation, conversation.photo)} alt={conversation.name} />
                                {:else}
                                    <div class="icon">
                                        <MdGroup />
                                    </div>
