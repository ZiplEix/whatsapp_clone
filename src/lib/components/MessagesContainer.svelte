                ...
                {#if messageGroup[0].user_id !== authModel.id}
                    <div class="message-image">
                        {#if messageUser && messageUser.photo}
                            <img src={$pb.files.getUrl(messageUser, messageUser.photo)} alt={messageUser.name}>
                        {:else}
                            <div class="icon">
                                <FaUserCircle />
                            </div>
                    ...
                    {#each messageGroup as message}
                        ...
                        >
                            {#if message.user_id === authModel.id && showRemoveButton === message.id}
                                <form
                                    action="{$page.url.pathname}?/removeMesage"
                                    method="post"
                            ...
                            <div class="message">
                                {#if message.contentType === "image"}
                                    <img src={$pb.files.getUrl(conversation, message.content)} alt={message.content}>
                                {:else}
