    ...
    $: if ($pb && browser) {
        if (searchText === "") {
            ...
            usersCollection.getFullList({
                ...
                if (searchText === localsSearchText) {
                    searchResults = validatedResult;
                }
            });
        }
    }
        ...
        try {
            ...
            if (!result.ok) {
                ...
                if (typeof message === 'string') {
                    ...
                    throw new Error("An unknow error occurred when creating conversation");
                }
            }
        } catch (error) {
            submitting = false;
...
<div class="app-container" class:app-container-full={!isModal}>
    <header>
        <div class="headings">
            ...
            {#if isModal}
                <button class="close-button" on:click={() => eventDispatcher("close")}>
                    <IoIosCloseCircle />
                </button>
            {/if}
        </div>
        ...
        <nav>
            <input type="text" bind:value={searchText} placeholder="Search by names or email">

            {#if selectedUsers.length > 0}
                <button transition:fade={{duration: 150}} on:click={handleSelectionClick}>
    ...
    <main>
        ...
        {#each showSelected ? selectedUsers : searchResults as user}
            ...
            <div class="list-item" class:selected={userSelected} transition:fade={{duration: 150}} on:click={() => handleClickUser(user)}>
                <div class="user-information">
                    ...
                    <button class:remove={userSelected} on:click|stopPropagation={() => handleClickUser(user)}>
                        <div class="icon">
                            {#if userSelected}
                                <IoMdClose />
                            {:else}
    ...
    <footer>
        ...
        <p>{selectedUsers.length} {selectedUsers.length === 1 ? "person": "people"} selected</p>

        <button disabled={selectedUsers.length === 0 || submitting} on:click={handleSubmit}>
            Continue
        </button>
    </footer>
</div>
